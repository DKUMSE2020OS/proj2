...
// 가상 메모리는 16bit address space(0~0xffff) 메인 메모리는 512KB(0~0x7ffff) 의 주소공간을 가짐
// page frame size 를 4KB가정, 따라서 offset은 12bit 을 가짐 , PFN은 0~127(128개)의 INDEX를 가진다.
...
void signal_handler(int signo)
{
	...
	int msgq2;
	 ...
	 while(PFN.count < 30){
                int flag = 0;
                fprintf(fp, "numbers of free page number are under 30!!!!!!!!!!!\n");
	...
	if (!IsEmpty(&run_q)) {
		...
		for(int i =0; i<10; i++){
			fprintf(fp, "        0x%04x -> ",msg2.random_address[i]);
		...
		//print LRU_STACK
		struct Node* pp = malloc(sizeof(Node));
		pp = LRU_STACK.top;
		fprintf(fp, "* LRU_STACK\n  ");
		if(pp == NULL){
			fprintf(fp, "(NULL)");
		}
		while(pp != NULL){
			fprintf(fp, "-%d", pp->data);
			pp = pp->link;
		}
		fprintf(fp, "\n");
		free(pp);
...
void Delete(free_PFN* LRU_S , Node* removenode){
	Node* newptr = LRU_S->top;
	while(newptr != NULL){
		if(removenode->data == newptr->data){
			if((newptr->b_link != NULL)&(newptr->link != NULL)){
				newptr->b_link->link = removenode->link;
				newptr->link->b_link = removenode->b_link;
			}
			//end
